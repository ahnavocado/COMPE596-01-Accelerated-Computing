...
void jacobi_gpu_simd(double *T, int N, int max_iter) {
    int iteration = 0;
    double residual = 1e6;
    ...
    while (residual > 1e-8 && iteration < max_iter) {
        ...
        #pragma omp target teams distribute parallel for collapse(2) simd reduction(max:residual)
        for (unsigned i = 1; i <= N; i++) {
            for (unsigned j = 1; j <= N; j++) {
    ...
    printf("Residual = %.9e\n", residual);
    printf("OpenMP GPU SIMD time = %.6f Sec\n", elapsed_time);
